...
const QuizHistory = () => {
    ...
    return (
        /* OPTIMIZATION: Added arbitrary Tailwind targets to mask scrollbars in Webkit engines (Chrome/Safari), IE, and Firefox */
        <div className='relative min-h-screen w-full bg-linear-to-br from-black via-slate-900 to-purple-950 flex justify-center items-start p-4 sm:p-6 md:p-10 pt-24 md:pt-28 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'>
            ...
            {selectedAttempt ? (
                <div className='relative z-10 w-full max-w-4xl rounded-3xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-2xl p-5 md:p-10 text-white flex flex-col'>
                    ...
                    <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-white/10 pb-6 mb-6 gap-4'>
                        ...
                        {/* Score Indicator */}
                        <div className='flex items-center gap-4 bg-white/5 border border-white/10 p-4 rounded-2xl self-start sm:self-auto'>
                ...
                <div className='relative z-10 w-full max-w-5xl rounded-3xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-2xl p-5 md:p-10 flex flex-col'>
                    ...
                    <div className='space-y-8'>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6'>
                            <div className='rounded-2xl bg-cyan-500/10 border border-cyan-500/20 p-6 md:p-8 text-center'>
                                <h2 className='text-4xl md:text-5xl font-extrabold text-cyan-400 mb-2'>
                                    {totalAttempts === 0 ? '—' : `${averagePercentage.toFixed(1)}%`}
                                </h2>
                                <p className='text-gray-300 text-sm md:text-base'>Overall Average</p>
                            </div>
                            <div className='rounded-2xl bg-purple-500/10 border border-purple-500/20 p-6 md:p-8 text-center'>
                                <h2 className='text-4xl md:text-5xl font-extrabold text-purple-400 mb-2'>{totalAttempts}</h2>
                                <p className='text-gray-300 text-sm md:text-base'>Total Attempts</p>
                            </div>
                        </div>
                        ...
                        {attempts.length === 0 ? (
                            ...
                            <div>
                                <h2 className='text-2xl font-bold text-white mb-6'>Previous Attempts</h2>
                                <div className='space-y-4'>
                                    {attempts.map((attempt) => (
                                        <div
                                            key={attempt.id || attempt._id}
                                            onClick={() => setSelectedAttempt(attempt)}
                                            className='cursor-pointer rounded-2xl border border-white/10 bg-white/5 backdrop-blur-lg p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:bg-white/10 hover:border-cyan-500/30 transition-all duration-300'
                                        >
